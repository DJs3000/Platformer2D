...
#include <chrono>                   // For crossplatform clock
#include <thread>                   // For crossplatform threads 
#include <cfloat>
...
//----------------------------------------------------------------------------------
static std::thread physicsThread;                           // Physics thread id
...
static void PhysicsLoop(void);                                                                        // Physics loop thread function
...
static void PhysicsLoop()
{
    ...
    while (physicsThreadEnabled)
    {
        ...
        RunPhysicsStep();
        std::this_thread::sleep_for(PHYSAC_FIXED_TIME * 1ms);
...
static void InitTimer(void)
{
    ...
    frequency    = static_cast<uint64_t>(period::den) / period::num;
